...
#
# description:
#   deploys a list of serverless configs in a specific order
...
CONFIGS_LOCATION="serverless"
DEPLOYMENT_ORDER=(
   serverless_api_gateway.yaml
   serverless_cloudwatch_logs_to_elasticsearch.yaml
   serverless_api_domain.yaml
)
...
for config in $configs; do
   ...
   # check for warnings
   if grep -Fq "$SLS_WARNING" "$TMP_FILE"; then
...
for config in $configs; do
   ...
   echo | tee -a "$LOG_FILE"
   command="$SLS_CMD $action --stage $stage $profile --config $CONFIGS_LOCATION/$config $verbose"
   ...
   if grep -Fq "$SLS_SKIPPING" "$TMP_FILE"; then
      stats[$i]="$SKIPPED"
   elif grep -Fq "$SLS_CREATE_FINISHED" "$TMP_FILE"; then
      stats[$i]="$CREATED"
   elif grep -Fq "$SLS_UPDATE_FINISHED" "$TMP_FILE"; then
      stats[$i]="$UPDATED"
   elif [ "$action" == "info" ]; then
      no_of_resources=$(grep -F "$SLS_RESOURCES" "$TMP_FILE" | awk '{print $NF}' | tr -d '[:cntrl:]')
   ...
   if grep -Fq "$SLS_WARNING" "$TMP_FILE"; then
      ...
      warns[$i]="$WARNING"
      [ "$(grep -Fc "$SLS_WARNING" "$TMP_FILE")" -gt 1 ] && warns[$i]=${warns[$i]/g/gs}
      exit_status=1
   fi

   # check for errors
   if grep -Fq "$SLS_ERROR" "$TMP_FILE"; then
      unset config_errors
   ...
   if grep -Fq "$SLS_CF_STACK" "$TMP_FILE"; then
      cf_stack_name=$(grep -F "$SLS_CF_STACK" "$TMP_FILE" | cut -d' ' -f2 | tr -d '[:cntrl:]')
...
if [ "$dry_run" != "true" ]; then
   ...
   for config in $configs; do
      ...
      [ -n "${warns[$i]}" ] && [ -n "${errs[$i]}" ] && wsnes="(${warns[$i]}, ${errs[$i]})"
      $ECHO_E "$(printf "${execs[$i]} %-${mcl}s - %${msl}s %${mcs}s %s" "${config%.yaml}" "${stats[$i]}" "${cf_stats[$i]}" "$wsnes")" | tee -a "$LOG_FILE"
      ((i++))
   done
   echo | tee -a "$LOG_FILE"

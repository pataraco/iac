output "dns_name" {
  ...
  value       = "${element(concat(aws_lb.network.*.dns_name, aws_lb.network_no_logs.*.dns_name), 0)}"
}
...
output "load_balancer_arn_suffix" {
  ...
  value       = "${element(concat(aws_lb.network.*.arn_suffix, aws_lb.network_no_logs.*.arn_suffix), 0)}"
}

output "load_balancer_id" {
  description = "The ID and ARN of the load balancer we created."
  value       = "${element(concat(aws_lb.network.*.id, aws_lb.network_no_logs.*.id), 0)}"
}

output "load_balancer_zone_id" {
  description = "The zone_id of the load balancer to assist with creating DNS records."
  value       = "${element(concat(aws_lb.network.*.zone_id, aws_lb.network_no_logs.*.zone_id), 0)}"
}
